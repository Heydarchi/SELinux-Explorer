...
class FilterUi(QHBoxLayout):
    ...
    def _init_widgets(self):
        ...
        self.btn_filter = UiUtility.create_button(
            "Generate output",
            QIcon(ICON_PATH + "filter.png"),
            self.BTN_WIDTH,
            self.BTN_HEIGHT,
        )
        self.btn_add_filter_rule = UiUtility.create_button(
            "Add a new filter",
            QIcon(ICON_PATH + "add.png"),
            self.BTN_WIDTH,
            self.BTN_HEIGHT,
        )
        ...
        self.tbl_rule.setColumnWidth(
            self.COL_EXACT_WORD_INDEX, self.COL_EXACT_WORD_WIDTH
        )
    ...
    def _config_layout(self):
        ...
        self.group_box.setMinimumWidth(self.TABLE_MIN_WIDTH + 3 * self.BTN_WIDTH)
    ...
    def on_get_filter(self, rule):
        ...
        self.tbl_rule.setItem(
            index, self.COL_TITLE_INDEX, QTableWidgetItem(rule.keyword.strip())
        )
        self.tbl_rule.setItem(
            index, self.COL_TYPE_INDEX, QTableWidgetItem(rule.filter_type.name.strip())
        )
        self.tbl_rule.setItem(
            index, self.COL_EXACT_WORD_INDEX, QTableWidgetItem(str(rule.exact_word))
        )
